...
class Original_KANLinear(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=2, num=5, k=3, noise_scale=0.1, scale_base=1.0, scale_sp=1.0, base_fun=torch.nn.SiLU(), grid_eps=0.02, grid_range=[-1, 1], sp_trainable=True, sb_trainable=True, device='cpu'):
        ...
        (3, 5)
        '''
        super(Original_KANLinear, self).__init__()
        # size 
    ...
    def initialize_grid_from_parent(self, parent, x):
        ...
        x_pos = parent.grid
        sp2 = Original_KANLinear(in_dim=1, out_dim=self.size, k=1, num=x_pos.shape[1] - 1, scale_base=0., device=self.device)
    ...
    def get_subset(self, in_id, out_id):
        ...
        (2, 3)
        '''
        spb = Original_KANLinear(len(in_id), len(out_id), self.num, self.k, base_fun=self.base_fun, device=self.device)
